...
const MyReviews = () => {
  ...
  const [mealsMap, setMealsMap] = useState({}); // foodId -> foodName
...
useEffect(() => {
  const fetchReviews = async () => {
    try {
      ...
      setReviews(res.data); // mealName is already included
    } catch (err) {
  ...
  const submitEdit = async () => {
    ...
    try {
      ...
      // Update review in state while keeping mealName
      setReviews(prev =>
        prev.map(r =>
        ...
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-xl w-full max-w-md shadow-2xl space-y-4">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setEditingReview(null)} className="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">
                Cancel
              </button>
              <button onClick={submitEdit} className="px-4 py-2 bg-emerald-500 text-white rounded hover:bg-emerald-600">
                Update
              </button>
            </div>
          </div>
